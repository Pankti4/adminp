  ...
  public function __construct() 
     {
      ...
      function index()
        ...
        $allcount = $this->Statemodel->get_count($search_text);

        $search_text = $this->input->get('searchKeyword');

        $config['base_url'] = base_url().'user/State/index?searchKeyword='.$search_text;
        $config['first_url'] = $config['base_url'];
         
        $config['total_rows'] = $this->Statemodel->get_count($search_text);
        ...
        if($this->input->get('per_page') && $this->input->get('per_page')!='1' ){
         ...
         // echo ($page); die();
         $data['states'] = $this->Statemodel->all($config["per_page"], $page, $search_text);
   ...
   function create()
   {
      ...
      {
         $country = $this->Statemodel->get_Country();
         $dt['country'] = $country->result();
         $this->load->view('user/screate',$dt); 
      } else 
      {
        $formArray = array();
